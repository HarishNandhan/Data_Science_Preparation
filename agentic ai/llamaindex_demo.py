...
class EuriaiLLM:
    ...
    def chat(self, messages: List[Any], **kwargs) -> Any:
        ...
        response_text = generate_completion(msg_payload)
...
# wrapper for the tools - we are keeping all the tools in a list
...
agent = ReActAgent.from_tools(
    ...
    # tool_choice_mode = "always" - means the agent will always use the tools
    # tool_choice_mode = "auto" - means the agent will use the tools if the tool is available
    # tool_choice_mode = "never" - means the agent will not use the tools
)
agent.chat("convert my name is harish nandhan in uppercase")
agent.chat("tell me something about what is the history of Labor Day in United States in wiki")
